...
const char lpAPPNAME[] = "G26P2SM4 Flash Loader";
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
struct ReqHS { unsigned __int64 guid; u32 crc; };
struct RspHS { unsigned __int64 guid; u32 crc; };
...
struct Request
{
	...
	{
		u32 func;
		struct { u32 func; u32 padr; u32 page[PAGESIZE>>2]; u32 crc; } f1;
		struct { u32 func; u32 data; u32 crc; } fx;
	};
};
...
struct Response
{
	...
	{
		u32 func;
		struct { u32 func; u32 padr; u32 status; u32 crc; } f1;
		struct { u32 func; u32 padr; u32 status; u32 crc; } fx;
	};
};
...
void UpdateRequest()
{
	...
	switch(i)
	{
		case 0:
			...
			req.f1.crc = GetCRC32(&req, sizeof(req) - sizeof(req.f1.crc));
		...
		case 2:
			...
			if (!com1.Update())
			{
				...
				if (!rb.recieved) { errornum++; };
	
				if (rb.len != sizeof(rsp.f1))
				{
					lenErrors++;
				}
				else if (CheckCRC32(&rsp, sizeof(rsp.f1)) != 0)
				{
					crcErrors++;
				};


				if (rb.recieved && rb.len == sizeof(rsp.f1) && CheckCRC32(&rsp, sizeof(rsp.f1)) == 0)
				{
					oknum++;
				...
				RECT rect = { 10, 80, 380, 180 };

				Printf(10, 80, "Запросов: %lu Таймаут: %lu Длина: %lu CRC: %lu", packetnum, errornum, lenErrors, crcErrors);
		...
		case 3:
			...
			req.fx.crc = GetCRC32(&req, sizeof(req.fx) - sizeof(req.fx.crc));
...
static void UpdateCom()
{
	...
	if (start) switch(i)
	{
		...
		case 2:
			...
			if (!com1.Update())
			{
				if (rb.recieved && rb.len == sizeof(reqHS) && CheckCRC32(&reqHS, sizeof(reqHS)) == 0 && reqHS.guid == slaveGUID)
				{
					rspHS.guid = masterGUID;
					rspHS.crc = GetCRC32(&rspHS, sizeof(rspHS) - sizeof(rspHS.crc));
					wb.data = &rspHS;
					wb.len = sizeof(rspHS);
					com1.Write(&wb);
					i++;
