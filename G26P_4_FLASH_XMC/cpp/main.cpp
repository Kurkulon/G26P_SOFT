...
const char lpAPPNAME[] = "G26P_4_XMC4800 Flash Loader";
			...
			if (!com1.Update())
			{
				if (rb.recieved && rb.len == sizeof(rsp.F3) && rsp.F3.padr == req.F3.padr && GetCRC16(&rsp, sizeof(rsp.F3)) == 0)
				{
					...
					if (rsp.F3.status != 0)
					{
						...
						if (secLen > PAGESIZE) { secLen -= PAGESIZE; } else	{ secLen = 0; };
...
void UpdateRequest()
{
	...
	switch(i)
	{
		case 0:
			...
			if (UpdateCheckEraseSector(true))
			{
				i++;
			};
		...
		case 1:
			...
			if (!UpdateCheckEraseSector(false))
			{
				if (pageAdr >= sizeof(flashPages))
				{
					i = 4;
				}
				else if (secLen == 0)
		...
		case 3:
			...
			if (!UpdateProgramSector(false))
			{
				...
				else if (pageAdr >= sizeof(flashPages))
				{
					i++;
				}
				else
