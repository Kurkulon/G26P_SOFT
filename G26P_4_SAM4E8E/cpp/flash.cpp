...
__packed struct Req
{
	...
	__packed union
	{
		__packed struct  { word crc; } f1;  // —тарт новой сессии
		__packed struct  { byte n; byte chnl; byte count[4]; byte time; byte gain; byte delay; byte filtr; u16 data[500]; word crc; } f2;  // «апись вектора
//		struct  { word crc; } f3;  // установка периода дискретизации вектора и коэффициента усилени€
	};
};
...
__packed struct Rsp
{
	...
	{
		__packed struct  { word crc; } f1;  // —тарт новой сессии
		__packed struct  { word crc; } f2;  // «апись вектора
		__packed struct  { word crc; } f3;  // 
		__packed struct  { word crc; } fFE;  // ќшибка CRC
		__packed struct  { word crc; } fFF;  // Ќеправильный запрос
	};
};
...
namespace Erase
{
	...
	static bool check = true;	// ѕроверить результат стирани€
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// провер€ет по указанному адресу
...
// пишет по указанному адресу смеща€сь на новы блок, возвращает исправленный адрес
...
//	{
//		//case NAND_STATE_WAIT:	// дл€ скорости
//		//	break;
...
//			if(current_adress_old != current_adress_new) // чтобы не считать за зр€
//			{
